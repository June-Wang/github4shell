...
usage (){
        ...
        echo "Usage: ${program_name} -v [6|7]" 1>&2
        exit 1
}
...
if [ ! -f "${file}" ]; then
        echo -n "download ${url} ...... "
        wget -q "${url}" -O /tmp/${file} && echo 'done.' || local download='fail'
...
decompress_file () {
local file="$1"
test -f /tmp/${file} && tar xzf /tmp/${file} || eval "echo ${file} not exsit!;del_tmp;exit 1"
}
...
#mv2path "${tomcat_path}"
#tomcat_path="/opt/${dst_path}"

#local tomcat_install_path="/home/${user}/${tomcat_path}"
local tomcat_install_path="/opt/${dst_path}"
...
#tomcat_env_profile="/home/${user}/.bash_profile"
...
case "${tomcat_version}" in
        6)
                ...
                ;;
        7)
                tomcat_path='apache-tomcat-7.0.59'
                ;;
        *)
